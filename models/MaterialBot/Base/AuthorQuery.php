...
use \Exception;
use \PDO;
use MaterialBot\Author as ChildAuthor;
use MaterialBot\AuthorQuery as ChildAuthorQuery;
use MaterialBot\Map\AuthorTableMap;
 ...
 *
 * @method     \MaterialBot\BookQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
 ...
 * @psalm-method ObjectCollection&\Traversable<ChildAuthor> findByFirstName(string $first_name) Return ChildAuthor objects filtered by the first_name column
 ...
 * @psalm-method ObjectCollection&\Traversable<ChildAuthor> findByLastName(string $last_name) Return ChildAuthor objects filtered by the last_name column
...
abstract class AuthorQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \MaterialBot\Base\AuthorQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'default', $modelName = '\\MaterialBot\\Author', $modelAlias = null)
    {
    ...
    /**
     * Filter the query by a related \MaterialBot\Book object
     *
     * @param \MaterialBot\Book|ObjectCollection $book the related object to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
    ...
    {
        if ($book instanceof \MaterialBot\Book) {
            return $this
            ...
            throw new PropelException('filterByBook() only accepts arguments of type \MaterialBot\Book or Collection');
        }
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \MaterialBot\BookQuery A secondary query class using the current class as primary query
     */
    public function useBookQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinBook($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Book', '\MaterialBot\BookQuery');
    }

    /**
     * Use the Book relation Book object
     *
     * @param callable(\MaterialBot\BookQuery):\MaterialBot\BookQuery $callable A function working on the related query
     *
     ...
     *
     * @return \MaterialBot\BookQuery The inner query object of the EXISTS statement
     */
     ...
     * @param string|null $queryClass Allows to use a custom query class for the exists query, like ExtendedBookQuery::class
     *
     * @return \MaterialBot\BookQuery The inner query object of the NOT EXISTS statement
     */
